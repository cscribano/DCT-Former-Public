...
class Sharding:
    ...
    def segment_articles_into_sentences(self, segmenter):
        ...
        if use_multiprocessing == "manager":
            ...
            jobs = []
            n_processes = 4  # in addition to the main process, total = n_proc+1

            def work(rank, articles, return_dict):
                sentences = {}
                if rank == 0:
                    for i, article in enumerate(tqdm(articles)):
                        si = segmenter.segment_string(articles[article])
                        if si is not None:
                            sentences[i] = si
                else:
            ...
            for rank, item in enumerate(chunks(self.articles, len(self.articles) // n_processes)):
                p = multiprocessing.Process(target=work, args=(rank, item, return_dict))
            ...
            for i, article in enumerate(tqdm(self.articles)):
                si = segmenter.segment_string(self.articles[article])
                if si is not None:
                    self.sentences[i] = si
                else:
                    self.forbidden_keys.append(i)
    ...
    def distribute_articles_over_shards(self):
        ...
        unused_article_set = set(k for k in self.articles.keys() if k not in self.forbidden_keys)
        ...
        while len(consumed_article_set) < (len(self.articles) - len(self.forbidden_keys)):
            ...
            print("\rDistributing data over shards:", len(unused_article_set), "articles remaining.", end='')
            if last_time_unused == len(unused_article_set):
                break

            last_time_unused = len(unused_article_set)

        print('\n')
        if len(unused_article_set) != 0:
            print(f"Warning: Some {len(unused_article_set)}  articles did not make it into output files.")

        for shard in self.output_training_files:
            print(
                "\rTraining shard:", self.get_sentences_per_shard(self.output_training_files[shard]),
                end=''
            )
        print('\n')
        for shard in self.output_test_files:
            print("\rTest shard:", self.get_sentences_per_shard(self.output_test_files[shard]), end='')
