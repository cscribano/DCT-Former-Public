...
class PretrainValidationDataset:
...
class PreTrainingDataset(BertDatasetProviderInterface):
    def __init__(self, cnf, bs_per_gpu, data_prefix="train"):
        ...
        self.train_micro_batch_size_per_gpu = bs_per_gpu #train_micro_batch_size_per_gpu
...
if __name__ == '__main__':
    ...
    """
    valid_ds = PretrainValidationDataset(cnf)
    dataset = valid_ds.get_validation_set(0)
    ...
    pretrain_dataset_provider = PreTrainingDataset(cnf, 1)
    ...
    for batch_index_number, batch_index in enumerate(dataset_iterator):
        try:
            ...
            token_type_ids = batch[3] (USELESS)
            next_sentence_label = batch[4]
            masked_lm_labels = batch[5] (NOT PRESENT)
            """
